...
import { getIsbn10, getIsbn13, getIsbnWithHyphen } from '@/utils/primitive.ts';

type Props = {
  book: BookData;
};

export default function IsbnTable({ book }: Props) {
    ...
    <div className="flex flex-col justify-between">
      {tableData.map((rowData, idx) => (
        <div key={idx} className="flex justify-between gap-3">
          ...
          <div className="text-right text-xs">{rowData.value}</div>
        </div>
      ))}
    </div>
  );
