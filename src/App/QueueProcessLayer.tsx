...
import {
  addFilterQueue, addGetImageQueue,
  completeFilterQueues,
  completeGetImageQueues, retryGetImageQueue,
...
const getBookImageQueueProcess = async (queuedBookImageIsbn: string[], lastEndTime: MutableRefObject<number>, bookImageLastResult: MutableRefObject<{ type: 'ndl' | 'other'; url: string | null | undefined } | null>) => {
  ...
  const list = await Promise.all(queuedBookImageIsbn.map((isbn) => new Promise<{ isbn: string, url: string | null }>((resolve) => {
    getBookImageUrl(isbn).then(({ url, error }) => {
